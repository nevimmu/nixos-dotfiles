...

{
  ...
  options = {
    mango.enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Enable MangoWC window manager configuration";
    };
  };

  config = lib.mkIf config.mango.enable {
    # Build the autostart script content
    wayland.windowManager.mango = let
      ...
      # Build the config.conf content
      configContent = ''
    # More options see https://github.com/DreamMaoMao/mango/wiki/
